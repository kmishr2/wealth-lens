...
func (s *Service) GetCurrent(userID uuid.UUID, portfolioID uuid.UUID) (PortfolioValuationResponse, error) {
	if _, err := s.getOwnedPortfolio(userID, portfolioID); err != nil {
		return PortfolioValuationResponse{}, err
	}
	...
	if err != nil {
		return PortfolioValuationResponse{}, common.BadRequest(err.Error())
	}
	...
	return ToResponse(portfolioID, holdingsResult, valuationResult), nil
}
